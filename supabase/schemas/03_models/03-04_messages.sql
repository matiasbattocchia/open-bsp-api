...
create trigger "handle_incoming_message_to_agent" after insert on "public"."messages" for each row when (("new"."direction" = 'incoming'::"public"."direction")) execute function "public"."edge_function"('/agent-client', 'post');
...
create trigger "notify_webhook_messages" after insert or update on "public"."messages" for each row execute function "public"."notify_webhook"();

create trigger "set_message" before update on "public"."messages" for each row when (("new"."message" is not null)) execute function "public"."merge_update_message"();

create trigger "set_status" before update on "public"."messages" for each row when (("new"."status" is not null)) execute function "public"."merge_update_status"();

create trigger "set_updated_at" before update on "public"."messages" for each row execute function "public"."moddatetime"('updated_at'); 